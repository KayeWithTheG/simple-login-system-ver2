...
if (isset($_GET['locked'])) {
    ...
    if ($lockData) {
        ...
        // If lockout already expired, do nothing extra — form stays unlocked.
    }
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && !$is_locked) {
    ...
    if (!empty($lockData['is_permanent'])) {
        ...
        // If previous lockout time has passed, reset attempts for this round
        // but keep level tracking
        if ($lockData['lockout_time'] > 0 && $currentTime >= $lockData['lockout_time']) {
            ...
            $lockData = $lockoutCollection->findOne([
                'student_id' => $student_id
            ]);
        }
        ...
        if (strlen($student_id) !== 9) {
            ...
            if ($valid) {
                ...
                // Successful login:
                // Delete lockout record so user starts fresh next time
                $lockoutCollection->deleteOne([
                    'student_id' => $student_id
                ]);
                ...
                if ($current_level === 0) {
                    ...
                    if ($current_level === 1) {
                        ...
                        $error = "Repeated failed attempts. Locked for 60 seconds.";
                        ...
                        $error = "Multiple failed attempts. Locked for 120 seconds.";
    ...
    if ($is_locked) {
        ...
        header(
            "Location: index.php?locked=" . urlencode($student_id)
        );
    ...
    <div class="box" style="width: 520px; padding: 40px;">
        ...
        <form method="POST" action="index.php">
            ...
            <input
                ...
                id="student_id"
                name="student_id"
                placeholder="Student ID (e.g. 25-266124)"
                maxlength="9"
                value="<?php echo htmlspecialchars($locked_student_id); ?>"
            ...
            >
                ...
                if ($is_permanent) {
                    ...
                    echo "Locked (<span id='countdown-timer'>"
                        . $remaining .
                        "</span>s)";
